...
const TodoistIndicator = GObject.registerClass(
  class TodoistIndicator extends PanelMenu.Button {
    ...
    _render() {
      let pastDueItems = this._tasks.filter(this._isDueDateInPast).filter( item => item.project_id !== 2070804635);
      let todayItems = this._tasks.filter(this._isDueDateToday).filter(item => item.project_id !== 2070804635);

      this.buttonText.set_text(this._getTextForTaskCount(pastDueItems.length + todayItems.length));
...
function _openTodoistWeb() {
  Util.spawn(['xdg-open', 'https://todoist.com/app/filter/2286495315']);
}
